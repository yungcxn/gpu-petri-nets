...
def make_output(places, transitions, trans_elems, marking, trans_arrays, uplinks, downlinks, uplink_startid, downlink_startid, target_uplink_startid):
  ...
  parts = [
    ...
    pack_uint32(uplinks),               # offset 16
    pack_uint32(downlinks),             # offset 20
    pack_uint32(uplink_startid),        # offset 24
    pack_uint32(downlink_startid),      # offset 28
    pack_uint32(target_uplink_startid), # offset 32
...
def gen_relay_race(elements, single_token=True, is_first_part=True):
  places = elements * 2
  if single_token:
    initial = [(1,1)] #if is_first_part else []
  else:
  ...
  indexes = list(range(elements * 2))
  indexes = indexes[1:-1] + [indexes[0]] + [indexes[-1]]
  ...
  if is_first_part:
    return make_output(places, elements * 2, padded_real_elems, marking, trans_arrays, 1, 1, 10, 100000, 1000_0000)
  else:
    return make_output(places, elements * 2, padded_real_elems, marking, trans_arrays, 1, 1, 1000_0000, 1000_1000, 10)
  ...
  if is_first_part:
    ...
    for i in range(n):
      trans_arrays.extend(make_transition_opt_place_indexed(
        padded_real_elems, [(1, i)], []))
  if is_first_part:
    return make_output(places, transitions, padded_real_elems, marking, trans_arrays, n, n, 10, 50000, 100000)
  else:
    return make_output(places, transitions, padded_real_elems, marking, trans_arrays, n, n, 100000, 500000, 10)
