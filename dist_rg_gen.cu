 ...
 * @my_pe: The PE ID of the current processing element
 * @target_pe: The PE ID of the target processing element
 * 
...
__device__ __forceinline__ void arc_publish(rg_arc_t* arcs, uint32_t from,
                                            ...
                                            uint32_t my_pe, uint32_t target_pe){
  rg_arc_t arc = {
    .from = from,
    .to = to,
    .transition_id = tid,
  ...
  uint32_t arc_idx;
  if (my_pe != target_pe) {
    arc_idx = nvshmem_uint32_atomic_fetch_add(queue_tail, 1, target_pe);
    nvshmem_quiet();
    nvshmem_putmem(&arcs[arc_idx], &arc, sizeof(rg_arc_t), target_pe);
    nvshmem_quiet();
  } else {
    arc_idx = atomicAdd(queue_tail, 1);
    arcs[arc_idx] = arc;
...
__device__ __forceinline__ void frontier_switch(frontier_t* dev_f) {
  int32_t old_idx = dev_f->current_idx;
  int32_t new_idx = 1 - old_idx;
  dev_f->heads[old_idx] = 0;
  dev_f->current_idx = new_idx;
  __threadfence_system();
}
...
__device__ __forceinline__ int32_t frontier_push_next(frontier_t* dev_f,
  ...
  if (my_pe != target_pe) {
    current_idx = nvshmem_int32_g(&(dev_f->current_idx), target_pe);
    next_idx = 1 - current_idx;
    head_next = nvshmem_int32_atomic_fetch_add(&(dev_f->heads[next_idx]), 1,
                                               target_pe);
    nvshmem_quiet();
    nvshmem_uint32_put(&(dev_f->front_buffers[next_idx][head_next]), &val, 1, 
                       target_pe);
    nvshmem_quiet();
  } else {
    current_idx = *((volatile int32_t*)&dev_f->current_idx); /* safety */
    next_idx = 1 - current_idx;
    head_next = atomicAdd(&(dev_f->heads[next_idx]), 1);
    dev_f->front_buffers[next_idx][head_next] = val;
    __threadfence_system();
  }
 ...
 */
static inline frontier_t* frontier_devcreate(const uint32_t maxnodes, 
                                             const uint32_t my_pe) {

  frontier_t* f = (frontier_t*) nvshmem_malloc(sizeof(frontier_t));
  uint32_t* front1 = (uint32_t*) nvshmem_malloc(maxnodes * sizeof(uint32_t));
  uint32_t* front2 = (uint32_t*) nvshmem_malloc(maxnodes * sizeof(uint32_t));
  cudaMemset(front1, 0, maxnodes * sizeof(uint32_t));
  cudaMemset(front2, 0, maxnodes * sizeof(uint32_t));
  frontier_t h_f;
  h_f.front_buffers[0] = front1;
  h_f.front_buffers[1] = front2;
  h_f.heads[0] = 0;
  h_f.heads[1] = 0;
  h_f.current_idx = 0;
...
static inline void frontier_devdestroy(frontier_t* f) {
  ...
  nvshmem_free(h_f.front_buffers[0]);
  nvshmem_free(h_f.front_buffers[1]);
  nvshmem_free(f);
}

/**
 * warp_aligned_copy_local - Copy data between warp-aligned buffers
 ...
 * If PEs both are 0, it performs a local copy;
 *
 * Return: void
 */
__device__ __forceinline__ 
void warp_aligned_copy_local(uint64_t* dst, const uint64_t* src, 
                             const uint32_t uint64s_per_n, const uint32_t lane){
...
__device__ __forceinline__ 
void warp_aligned_copy_to_remote(uint64_t* dst, const uint64_t* src, 
                                 const uint32_t uint64s_per_n, 
                                 const uint32_t lane, const uint32_t my_pe,
                                 const uint32_t target_pe){
  if (my_pe != target_pe) {
    if (lane == 0) {
      nvshmem_putmem(dst, src, uint64s_per_n * sizeof(uint64_t), target_pe);
    }
    nvshmem_quiet();
    __syncwarp();
  } else {
...
/**
 * warp_arrays_equal_remote - Check if two arrays are equal using warps
 * @a: First array 
 * @b: Second array (Remote Node / Symmetric Address)
 ...
 */
__device__ __forceinline__ 
bool warp_arrays_equal_remote(const uint64_t* a, const uint64_t* remoteb,
                              const uint32_t count, const uint32_t lane,
                              const uint32_t my_pe, const uint32_t target_pe) {
  bool is_equal = true;
  if (my_pe != target_pe) {
    for (uint32_t i = lane; i < count; i += 32) {
      uint64_t val_b = nvshmem_uint64_g(&remoteb[i], target_pe); 
      if (a[i] != val_b) {
        is_equal = false;
      }
    }
...
/**
 * glob_sync - Global barrier synchronization across all threads
 ...
 * Return: void
 */
__device__ __forceinline__ void glob_sync(uint32_t* dev_sync_data,
 ...
 * @dev_f: Pointer to the frontier structure on device
 * @my_pe: ID of the current processing element
 * @n_pes: Total number of processing elements
 *
...
__global__ void insert_initial_node_kernel(void* dev_nodes_arr,
  ...
  const uint32_t target_pe = initial_hash % n_pes;
  ...
  if (my_pe == target_pe) {
    ...
    if (lane == 0) {
      ...
      dev_f->front_buffers[0][0] = (uint32_t)initial_hash;
      dev_f->heads[0] = 1;
    }
    __threadfence_system();
    __syncwarp();
...
#define DO_ONCE_INGROUP(X) { if (worker == 0) { X; } __syncthreads(); }
  ...
  *p = '\0';
  ...
  while (true) {
    ...
    const int32_t curr_idx = dev_f->current_idx;
    const int32_t curr_head = dev_f->heads[curr_idx];

    for (uint32_t front_idx = group; front_idx < curr_head; 
         front_idx += total_groups) {

      // if (atomicAdd(dev_global_runflag, 0) == 0) break;

      DO_ONCE_INGROUP({
        *shared_v = dev_f->front_buffers[curr_idx][front_idx];
        DEBUG_PRINTF("(%s) Front idx: %u, Node idx: %u, head_now: %d, "
                     "nodes_ptr: %p, offset: %lu, address: %p\n", 
                     transition_name, front_idx, *shared_v, curr_head,
                     nodes_ptr, (*shared_v) * bytes_per_node,
                     (void*)((uint8_t*)nodes_ptr + (*shared_v)*bytes_per_node));
      });
      ...
      for (uint32_t tid = warp_in_group; tid < rg_data.transitions; 
        ...
        warp_aligned_copy_local(warp_marking_buf_64, c_node_u64,
                                uint64s_per_node, lane);
...
#endif 
        const uint32_t target_pe = new_node_hash % n_pes;                       
        ...
        for (uint32_t probe = 0; probe < maxnodes; probe += 1) {
          ...
          do {
            ...
            DO_ONCE_INWARP({
              if (my_pe != target_pe) {
                lock_state = nvshmem_uint_atomic_compare_swap(target_lock,
                                                              LOCK_FREE,
                                                              LOCK_WRITING,
                                                              target_pe);
                nvshmem_quiet();
              } else {
                lock_state = atomicCAS(target_lock, LOCK_FREE,  
                                       LOCK_WRITING);
                __threadfence_system();
              }
          ...
          if (lock_state == LOCK_WRITTEN) {
            ...
            bool is_equal = warp_arrays_equal_remote(warp_marking_buf_64, 
                                                     target_node_ptr_64,
                                                     uint64s_per_node, lane,
                                                     my_pe, target_pe);
            if (is_equal) {
              DO_ONCE_INWARP(arc_publish(rg_data.dev_arcs_arr, *shared_v,
                                         ...
                                         rg_data.maxarcs, my_pe, target_pe));
          ...
          if (lock_state == LOCK_FREE) {
            ...
            DO_ONCE_INWARP({
              if (my_pe != target_pe) {
                old_node_count = 
                  nvshmem_uint_atomic_fetch_add(dev_created_nodes, 1,
                                                target_pe);
                nvshmem_quiet();
              } else {
            ...
            if (old_node_count >= rg_data.maxnodes) {
              DO_ONCE_INWARP({
                /* notify warp and break */
                if (my_pe != target_pe) {
                  nvshmem_uint_atomic_set(target_lock, LOCK_FREE, target_pe);
                  nvshmem_quiet();
                } else {
              ...
              *dev_global_runflag = 0xFFFFFFFF; /* stop all */
              break;
            }

            warp_aligned_copy_to_remote(target_node_ptr_64, warp_marking_buf_64, 
                                        uint64s_per_node, lane, my_pe, 
                                        target_pe);

            DO_ONCE_INWARP({
              if (my_pe != target_pe) {
                nvshmem_uint_atomic_set(target_lock, LOCK_WRITTEN, target_pe);
                nvshmem_quiet();
              } else {
                atomicExch(target_lock, LOCK_WRITTEN);
                __threadfence_system();
              }
              
              frontier_push_next(dev_f, target_node_idx, my_pe, target_pe);
              DEBUG_PRINTF("(%s) TID %d: Created new node %u\n", 
                           transition_name, tid, target_node_idx);

              arc_publish(rg_data.dev_arcs_arr, *shared_v,
                          target_node_idx, tid, dev_created_arcs,
                          rg_data.maxarcs, my_pe, target_pe);
            });
    ...
    { /* end phase? */
      ...
      __threadfence_system();
      DO_ONCE(nvshmem_barrier_all());
      glob_sync(dev_global_sync_lock, total_groups * blockDim.x, local_sense);
      /* now, all threads stopped producing into local and remote frontier */

      DO_ONCE({
        DEBUG_PRINTF("(%s) %d passed first barrier, doing switch\n", 
                    transition_name, my_pe);
        DEBUG_PRINTF("(%s) Before switch, current_idx=%d, head_now=%d, "
                    "head_next=%d\n",
                    transition_name, dev_f->current_idx,
                    dev_f->heads[dev_f->current_idx],
                    dev_f->heads[1 - dev_f->current_idx]);
        frontier_switch(dev_f); 
        DEBUG_PRINTF("After switch: head_now=%d, head_next=%d\n", 
                    dev_f->heads[dev_f->current_idx],
                    dev_f->heads[1 - dev_f->current_idx]);
        uint32_t* local_work_ptr = (uint32_t*)&(dev_f->heads[dev_f->current_idx]);
        nvshmem_uint32_max_reduce(NVSHMEM_TEAM_WORLD, dev_global_runflag, 
                                  local_work_ptr, 1);
        nvshmem_quiet();
        DEBUG_PRINTF("(%s) %d reduced runflag to %u\n", 
                    transition_name, my_pe, *dev_global_runflag);
      });      

#ifdef DEBUG
      DO_ONCE({
        DEBUG_PRINTF("(%s) %d reaching second barrier after switch\n", 
                    transition_name, my_pe);
      });
#endif
...
#ifdef DEBUG
      DO_ONCE({
        DEBUG_PRINTF("(%s) %d passed last barrier, reading runflag\n", 
                     transition_name, my_pe);
      });
#endif

      if (*dev_global_runflag == 0 || *dev_global_runflag == 0xFFFFFFFF) break; 
    }
  }
}
 ...
 * @my_pe: the PE id of this process
 * @n_pes: the total number of PEs in this run
 *
...
static uint8_t dissect_netdata_run_rg_gen(device_prop_t* dev_props,
  ...
  /* first nvshmem array */
  rg_data.dev_nodes_arr = nvshmem_malloc(rg_data.bytes_per_node 
                                         * rg_data.maxnodes);
  ...
  /* second nvshmem array */
  rg_data.dev_arcs_arr = (rg_arc_t*) nvshmem_malloc(rg_data.maxarcs 
                                                    * sizeof(rg_arc_t));
  ...
  /* third nvshmem allocation */
  uint32_t* dev_created_nodes = (uint32_t*) nvshmem_malloc(sizeof(uint32_t));
  ...
  /* global tracker for the number of created arcs for dev_arcs_arr, */
  uint32_t* dev_created_arcs = (uint32_t*) nvshmem_malloc(sizeof(uint32_t));
  ...
  /* fourth nvshmem allocation */
  uint32_t* dev_nodewrite_locks = (uint32_t*) nvshmem_malloc(sizeof(uint32_t) 
                                                             *rg_data.maxnodes);
  ...
  /* fifth nvshmem allocation */
  uint32_t* dev_global_runflag = (uint32_t*) nvshmem_malloc(sizeof(uint32_t));
  ...
  frontier_t* dev_f = frontier_devcreate(rg_data.maxnodes, my_pe);

  cudaStreamSynchronize(io_stream);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("CUDA Error after insert kernel: %s\n", cudaGetErrorString(err));
    exit(1);
  }
  nvshmem_barrier_all();

  void* insert_kernel_args[] = {
    &rg_data.dev_nodes_arr,
    &managed_marking_ptr,
    &dev_nodewrite_locks,
  ...
  nvshmemx_collective_launch((void*)insert_initial_node_kernel, dim3(1),
                             dim3(32), insert_kernel_args, 0, io_stream);
  ...
  if (net_type == NET_TYPE_BE) {
    if (use_glob_pool) {
      nvshmemx_collective_launch((void*)rg_transition_kernel<be_net_t, false>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);

    } else {
      nvshmemx_collective_launch((void*)rg_transition_kernel<be_net_t, true>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);
    ...
    if (use_glob_pool) {
      nvshmemx_collective_launch((void*)rg_transition_kernel<pt_net_t, false>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);

    } else {
      nvshmemx_collective_launch((void*)rg_transition_kernel<pt_net_t, true>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);
    ...
    if (use_glob_pool) {
      nvshmemx_collective_launch((void*)rg_transition_kernel<kk_net_t, false>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);

    } else {
      nvshmemx_collective_launch((void*)rg_transition_kernel<kk_net_t, true>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);
    }
  } else if (net_type == NET_TYPE_MINIBE) {
    if (use_glob_pool) {
      nvshmemx_collective_launch((void*)rg_transition_kernel<minibe_net_t,
                                 false>, dim3(min_grid_sz), 
                                 dim3(opt_block_size), kernel_args, 
                                 shared_mem_size, compute_stream);
    } else {
      nvshmemx_collective_launch((void*)rg_transition_kernel<minibe_net_t,true>,
                                 dim3(min_grid_sz), dim3(opt_block_size),
                                 kernel_args, shared_mem_size, compute_stream);
    }
  } else {
  ...
  nvshmem_free(dev_global_runflag);
  nvshmem_free(dev_created_nodes);
  nvshmem_free(dev_created_arcs);
  nvshmem_free(dev_nodewrite_locks);
  ...
  nvshmem_free(rg_data.dev_nodes_arr);
  nvshmem_free(rg_data.dev_arcs_arr);
...
int main(int argc, char* argv[]) {
  ...
  err = cudaSetDevice(0);
  if (err != cudaSuccess) {
    fprintf(stderr, "Rank %d: Failed to set device %d: %s\n", 
            mpi_rank, 0, cudaGetErrorString(err));
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  /*********************************************************** NVSHMEM init ***/
  nvshmemx_init_attr_t attr;
  memset(&attr, 0, sizeof(nvshmemx_init_attr_t));
  MPI_Comm comm = MPI_COMM_WORLD;
  attr.mpi_comm = &comm;
  nvshmemx_init_attr(NVSHMEMX_INIT_WITH_MPI_COMM, &attr);

  uint32_t my_pe = nvshmem_my_pe();
  uint32_t n_pes = nvshmem_n_pes();
  ...
  snprintf(nodes_out_file, sizeof(nodes_out_file), "%s-nodes.p%d", fileout,
           my_pe);
  snprintf(arcs_out_file, sizeof(arcs_out_file), "%s-arcs.p%d", fileout, my_pe);
  ...
  dissect_netdata_run_rg_gen(&dev_props, host_managed_data, maxnodes, maxarcs,
                             nodes_out, arcs_out, my_pe, n_pes);
